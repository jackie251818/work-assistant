...
- **单条提醒置顶** — 鼠标悬停任意任务点右侧「📌」，即可把这条提醒固定在列表最前面（再点一次取消）；多条置顶时按提醒时间排序，已完成任务当天自动沉底、次日回到最前；设置页任务卡片也可操作
...
- **点击 — 按钮** — 最小化到任务栏（任务栏显示应用图标，点击图标恢复面板）
- **点击 ▲ 按钮** — 收起为迷你长条（340×34），只保留日期、待办统计和下一条任务预览；再点 ▼ 或鼠标悬停可展开
- **按住面板头部拖动** — 移动面板位置（仅在解锁状态下可用）
- **点击任务左侧圆圈** — 标记完成 / 取消完成
- **面板空白处右键**（展开态）— 弹出快捷菜单，可快速勾选/取消「窗口置顶」

### 收起 / 展开

> 可在设置 → 偏好设置中通过「启用收缩功能」开关关闭自动收展；关闭后仅停用悬停自动展开与空闲自动收起，仍可随时点 `▲` / `▼` 手动收起、展开。
...
- 设置项 `alwaysOnTop`（默认 `true`），主进程 `applyAlwaysOnTop()` 在启动时和设置变更时调用 `mainWindow.setAlwaysOnTop()`
- 设置 → 偏好设置 → 「窗口置顶」开关；关闭后面板会被其他普通窗口遮挡，可正常切到后台
...
- 修复：记事本编辑时面板会因空闲自动收起而被收起 — 进入记事本视图后取消自动收起计时，切回待办视图时恢复，避免编辑被打断
- 优化：移除展开态面板空白处右键弹出的「窗口置顶」快捷菜单（窗口置顶仍可在设置 → 偏好设置中开关）

### v1.5.0 (2026-09-15)

- **新增：单条提醒置顶** — 悬停任务点右侧「📌」即可把该提醒固定在今日列表最前面（再点取消）；多条置顶按提醒时间排序，任务完成当天自动沉底、次日仍保持置顶；设置页任务卡片同步提供「📌 置顶」按钮
...
- 修复：头部新增记事本按钮后日期文字被挤换行的问题（缩小头部按钮与间距，日期区禁止换行）

### v1.4.0 (2026-09-13)

- **新增：窗口置顶** — 面板始终保持在其他窗口上方不被遮挡；设置 → 偏好设置新增「窗口置顶」开关（默认开启），关闭后面板可被普通窗口覆盖
- **新增：最小化到任务栏** — 面板头部新增「—」按钮，点击后面板最小化并在任务栏显示图标，点任务栏图标一键恢复；系统误触发的最小化仍会被自动还原
- **新增：右键快捷菜单** — 展开态下面板内右键弹出菜单，可一键切换窗口置顶（勾选态实时同步），左键点菜单外或按 Esc 关闭
- 优化：收起/展开按钮改为方向三角，语义更直观 — 展开态显示 `▲`（点击收起为迷你长条），收起长条显示 `▼`（点击展开面板）
- 优化：「启用收缩功能」关闭后仍可手动点 `▲` / `▼` 收起展开，仅停用悬停自动展开与空闲自动收起
- 修复：preload 中 `hidePanel` 误用 `ipcMain` 的隐患
